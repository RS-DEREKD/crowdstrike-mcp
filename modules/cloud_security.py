...
class CloudSecurityModule(BaseModule):
    ...
    def __init__(self, client):
        ...
        if not any([self._cloud_security, self._detections, self._assets]):
            raise ImportError(
                "No cloud security FalconPy classes available. "
    ...
    def register_resources(self, server: FastMCP) -> None:
        from resources.fql_guides import CLOUD_RISKS_FQL, CLOUD_IOM_FQL, CLOUD_ASSETS_FQL
        ...
        for uri, content in [
            ...
            server.resource(uri, name=uri.split("/")[-1], description=f"FQL syntax: {uri}")(
                _make_fn(content)
            )
    ...
    def register_tools(self, server: FastMCP) -> None:
        self._add_tool(
            server, self.cloud_get_risks, name="cloud_get_risks",
            description=(
        ...
        self._add_tool(
            server, self.cloud_get_iom_detections, name="cloud_get_iom_detections",
            description=(
        ...
        self._add_tool(
            server, self.cloud_query_assets, name="cloud_query_assets",
            description=(
        ...
        self._add_tool(
            server, self.cloud_compliance_by_account, name="cloud_compliance_by_account",
            description=(
                "Get compliance posture aggregated by account and region. "
                "Shows compliance control results with resource counts and severities."
            ),
        )
            ...
            for i, a in enumerate(assets, 1):
                ...
                if a.get("relationships"):
                    for rel in a["relationships"][:5]:
                        lines.append(
                            f"   Relationship: {rel.get('relationship_name', '')} "
                            f"{rel.get('resource_type', '')} ({rel.get('resource_id', '')})"
                        )
                lines.append("")
    ...
    async def cloud_compliance_by_account(
        ...
        if not entries:
            ...
            for i, c in enumerate(entries, 1):
                lines.append(
                    f"{i}. [{c['cloud_provider']}] Account: {c['account_id']} Region: {c['region']}"
                )
                lines.append(
                    f"   Service: {c['service']} ({c['service_category']}) | Resource: {c['resource_type']}"
                )
        ...
        try:
            ...
            for e in entities:
                ...
                detections.append({
                    ...
                    "compliance_controls": [
                        f"{c.get('framework', '')}: {c.get('name', '')} ({c.get('requirement', '')})"
                        for c in rule.get("controls", [])[:5]
                    ],
